...
Description &The respected actor will log in to the application by providing the assigned role, user name, and password. After clicking the login button, the application will check for its validity. If this actor data exists in the database, the application will show the respective actor dashboard to them.
The application will show an error message if the entered data is not matched with the database. \\ \hline
Pre-Condition & The respective actor will initiate the application.  \\
\hline
Flow & Main Scenario:
\begin{enumerate}
\item Actor loads the application, and the login form is displayed.
\item Actor selects their respective role from the combo box. 
\item Actor enters their assigned username in the textbox.
\item Actor enters their password in the textbox.
\item After entering the data, the actor clicks on the login button. 
\end{enumerate}
Alternate Flow:
\begin{itemize}
